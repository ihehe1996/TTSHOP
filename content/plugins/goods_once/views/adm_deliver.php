...
$secrets = $db->fetch_all("SELECT s.content, u.create_time
...
<div class="deliver-page">
    ...
    <div class="deliver-card deliver-form">
        <h3>填写发货内容</h3>
        <textarea id="deliver-content" placeholder="每行一条卡密内容，例如账号、密码、兑换码等"></textarea>
...
layui.use(['layer'], function() {
    ...
    $('#deliver-submit').on('click', function(){
        ...
        $.ajax({
            url: '<?= PLUGIN_URL ?>goods_once/api.php?action=deliver',
            type: 'POST',
            dataType: 'json',
            data: {
                order_id: orderId,
