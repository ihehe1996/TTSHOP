...
<form class="layui-form " action="" id="form">
    <div style="padding: 25px;" id="open-box">
        <div class="layui-form-item">
            <label class="layui-form-label">主站分类名</label>
            <div class="layui-input-block">
                <input type="text" name="sortname" disabled readonly class="layui-input" value="<?= $sort['sortname'] ?>">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">自定义名称</label>
            <div class="layui-input-block">
                <input type="text" name="custom_name" class="layui-input" value="<?= empty($station_sort) ? '' : $station_sort['custom_name'] ?>">
            </div>
        </div>
...
<script>
    layui.use(['table'], function(){
        ...
        form.on('submit(submit)', function(data){
            ...
            $.ajax({
                type: "POST",
                url: "?action=master_sort_edit_ajax",
                data: field,
                dataType: "json",
                success: function (e) {
                    if(e.code == 400){
