...
<script>
    layui.use(['table'], function(){
        ...
        window.table = table.render({
            elem: '#index',
            autoSort: false,
            url: '?action=level_index', // 此处为静态模拟数据，实际使用时需换成真实接口
            toolbar: '#toolbar',
            limits: [],
            page: false,
            ...
            cols: [[
                {type: 'checkbox'},
                {field:'name', title:'等级名称', minWidth: 170 },
                {field:'price', title:'开通价格', minWidth: 100 },
                {field:'is_domain', title:'独立域名', minWidth: 100, templet: '#is_domain' },
                // {field:'is_goods', title:'供货权限', minWidth: 100, templet: '#is_goods' },
                // {field:'is_station', title:'分站权限', minWidth: 100, templet: '#is_station' },
                {field:'cash_change', title:'提现手续费', minWidth: 100 },
                {title:'操作', templet: '#operate', width: 160, align: 'center'}
            ]],
        ...
        form.on('switch(switch-isdomain)', function(obj){
            var active = obj.elem.checked == true ? 'y' : 'n';
            var id = this.name;
            var loadSwitch = layer.load(2);
            $.ajax({
                url: '?action=level_switch_isdomain',
                type: 'POST',
                dataType: 'json',
                data: { id: id, status: active, token: '<?= LoginAuth::genToken() ?>' },
                success: function(e) {
                    if(e.code == 400){
                        layer.msg(e.msg)
                    }else{
        ...
        form.on('switch(switch-isstation)', function(obj){
            var active = obj.elem.checked == true ? 'y' : 'n';
            var id = this.name;
            var loadSwitch = layer.load(2);
            $.ajax({
                url: '?action=level_switch_isstation',
                type: 'POST',
                dataType: 'json',
                data: { id: id, status: active, token: '<?= LoginAuth::genToken() ?>' },
                success: function(e) {
        ...
        form.on('switch(switch-isgoods)', function(obj){
            var active = obj.elem.checked == true ? 'y' : 'n';
            var id = this.name;
            var loadSwitch = layer.load(2);
            $.ajax({
                url: '?action=level_switch_isgoods',
                type: 'POST',
                dataType: 'json',
                data: { id: id, status: active, token: '<?= LoginAuth::genToken() ?>' },
                success: function(e) {
        ...
        table.on('toolbar(index)', function(obj){
            ...
            if(obj.event == 'add'){
                ...
                let area = isMobile ? ['98%', '80%']  : ['700px', '400px'];
                layer.open({
                    id: 'add',
                    title: '添加分站等级',
                    type: 2,
...
<script>
    ...
    $('#menu-station-level > a').attr('class', 'menu-link active')
</script>
