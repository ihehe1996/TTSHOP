    ...
    <div style="padding: 25px;" id="open-box">
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="title" name="title" class="layui-input" value="<?= $tpl['group_name'] ?>">
            </div>
        </div>
        ...
        <blockquote class="layui-elem-quote layui-quote-nm" id="sku-div-<?= $key ?>">
            <div class="layui-form-item">
                ...
                <div class="layui-input-group">
                    ...
                    <input type="hidden" name="name[<?= $key ?>][old]" value="<?= $val['spec_name'] ?>" placeholder="如：颜色、尺码" class="layui-input">
                    <input type="text" name="name[<?= $key ?>][new]" value="<?= $val['spec_name'] ?>" placeholder="如：颜色、尺码" class="layui-input">
                    <div class="layui-input-suffix">
                        <span class="layui-btn layui-btn-blue add-value-btn" data-id="<?= $key ?>">添加规格值</span>
                        <span class="layui-btn layui-btn-red del-attr-btn" data-id="0">删除规格</span>
                    </div>
                </div>
            </div>
            <?php foreach($sku_value as $v): ?>
            <?php if($val['id'] == $v['spec_id']): ?>
            <div class="layui-inline" style="margin-top: 8px;">
                <div class="layui-input-inline">
                    <div class="layui-input-group">
                        ...
                        <input type="hidden" value="<?= $v['option_name'] ?>" name="value[<?= $key ?>][old][]" placeholder="如：白色、黑色" class="layui-input">
                        <input type="text" value="<?= $v['option_name'] ?>" name="value[<?= $key ?>][new][]" placeholder="如：白色、黑色" class="layui-input">
                        <div class="layui-input-suffix">
