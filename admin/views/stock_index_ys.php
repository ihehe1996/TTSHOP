...
<div style="padding: 20px 10px;" id="open-box">
    ...
    <table class="layui-hide" id="stock_index_ys" lay-filter="stock_index_ys"></table>
</div>
...
<script type="text/html" id="operate">
    <div class="layui-clear-space">
        ...
        <a class="layui-btn layui-btn-xs layui-btn-red" lay-event="del">删除</a>
    </div>
</script>
...
<script>
    layui.use(['table'], function(){
        ...
        window.table = table.render({
            elem: '#stock_index_ys',
            autoSort: false,
            url: 'stock.php?action=index_ys_ajax&goods_id=<?= $goods_id ?>', // 此处为静态模拟数据，实际使用时需换成真实接口
            // toolbar: '#toolbar',
            ...
            cols: [[
                ...
                {field:'create_time', title:'出售时间', sort: true, minWwidth: 150, align: 'center'},
        ...
        // 工具栏事件
        table.on('toolbar(stock_index_ys)', function(obj){
        ...
        // 触发单元格工具事件
        table.on('tool(stock_index_ys)', function(obj){ // 双击 toolDouble
            ...
            if(obj.event === 'edit'){
                ...
                layer.open({
                    ...
                    content: 'user.php?action=edit&uid=' + data.uid,
                    fixed: false, // 不固定
                    maxmin: true,
                    shadeClose: true,
